...
const HASHES_FILE = path.join(__dirname, 'backend_hashes.json');
...
const transporter = nodemailer.createTransport({
  service: 'gmail',
  auth: {
    user: process.env.EMAIL_USER || 'praveenvenkatesh.link@gmail.com',
...
app.post('/api/check-expiry', async (req, res) => {
  try {
    ...
    if (diffDays <= 30) {
      const mailOptions = {
        from: process.env.EMAIL_USER || 'praveenvenkatesh.link@gmail.com',
        to: `23cc043@nandhaengg.org, ${process.env.EMAIL_USER || 'praveenvenkatesh.link@gmail.com'}`,
